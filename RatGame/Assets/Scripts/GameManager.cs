...
public class GameManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI LeftDays;
    ...
    public void OnCutScene(int cutscenenumber)
    {
        CutScenes.gameObject.SetActive(true);
        CutScenes.GetChild(cutscenenumber).gameObject.SetActive(true);
        CutScenes.GetChild(cutscenenumber).GetComponent<Animator>().SetTrigger("On");
    }
    ...
    public void Closecutscene(int cutscenenumber)
    {
        CutScenes.gameObject.SetActive(false);
        CutScenes.GetChild(cutscenenumber).gameObject.SetActive(false);
    }
    ...
    public void RealOnScreen()
    {
        ...
        switch (nowscreen)
        {
            ...
            //    break;
            case ScreenType.침실:
                LeftDays.text = $"※  보고서 제출까지 {((2 - Day) % 7 + 7) % 7}일 남았습니다.";//1 -> 1, 2 -> 1, 3 -> 6, 4 -> 5, 5 -> 4, 6 -> 3, 7 -> 2, 8 -> 1, 9 -> 0, 10 -> 6
                break;
